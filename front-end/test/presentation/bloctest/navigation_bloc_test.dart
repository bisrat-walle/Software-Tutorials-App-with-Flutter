  ...
  final mockAuthenticationBloc = MockAuthenticationBloc();
   group('NavigationBloc', () {
    ...
    blocTest<NavigationBloc, NavigationState>(
     ...
     setUp:() {
    when(mockAuthenticationBloc.state).thenAnswer((realInvocation) => Authenticated(),);
    when(mockAuthenticationRepository.getLoginStatus()).thenAnswer((realInvocation) => Future.value(true));
     },
	...
	blocTest<NavigationBloc, NavigationState>(
     ...
     setUp:() {
		when(mockAuthenticationBloc.state).thenAnswer((realInvocation) => UnAuthenticated(),);
    when(mockAuthenticationRepository.getLoginStatus()).thenAnswer((realInvocation) => Future.value(false));
     },
      wait: const Duration(seconds: 3),
      act: (bloc) async => bloc.add(NavigationInitialEvent()),
      build: () => NavigationBloc(mockAuthenticationBloc),
	...
	blocTest<NavigationBloc, NavigationState>(
     ...
     act: (bloc) async => bloc.add(GotoEnrolledTutorials()),
      build: () => NavigationBloc(mockAuthenticationBloc)..add(GotoEnrolledTutorials()),
