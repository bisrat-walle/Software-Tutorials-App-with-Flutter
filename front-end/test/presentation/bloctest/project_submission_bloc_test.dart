...
void main() {
  ...
  group('ProjectBloc', () {
    blocTest<ProjectBloc, ProjectState>(
      ...
      setUp: () {
        when(mockTutorialRepository.getTutorial(tutorial.tutorialId))
            .thenAnswer((realInvocation) => Future.value(tutorial));
      },
      build: () => ProjectBloc(mockTutorialRepository, mockProjectRepository)
        ..add(LoadTutorialEvent(tutorial.tutorialId!)),
    ...
    blocTest<ProjectBloc, ProjectState>(
      ...
      setUp: () {
        ...
        when(mockProjectRepository.createProject(
                tutorialId: 1, projectUrl: "projectUrl"))
            .thenAnswer((_) => Future.value(true));
      },
      build: () => ProjectBloc(mockTutorialRepository, mockProjectRepository)
        ..add(ProjectCreation(tutorial.tutorialId!, "projectUrl")),
      expect: () =>
          [ProjectSubmissionLoading(), TutorialFetching(), expectedState],
    );
    ...
    blocTest<ProjectBloc, ProjectState>(
      ...
      setUp: () {
        ...
        when(mockTutorialRepository.getTutorial(tutorial.tutorialId))
            .thenAnswer((realInvocation) => Future.value(tutorial));
        when(mockProjectRepository.updateProject(
                tutorialId: 1, projectUrl: "projectUrl"))
            .thenAnswer((_) => Future.value(true));
      },
      build: () => ProjectBloc(mockTutorialRepository, mockProjectRepository)
        ..add(ProjectUpdate(tutorial.tutorialId!, "projectUrl")),
      expect: () =>
          [ProjectSubmissionLoading(), TutorialFetching(), expectedState],
    );
    ...
    blocTest<ProjectBloc, ProjectState>(
      ...
      setUp: () {
        ...
        when(mockTutorialRepository.getTutorial(tutorial.tutorialId))
            .thenAnswer((realInvocation) => Future.value(tutorial));
        when(mockProjectRepository.deleteProject(tutorialId: 1))
            .thenAnswer((_) => Future.value(true));
      },
      build: () => ProjectBloc(mockTutorialRepository, mockProjectRepository)
        ..add(ProjectDelete(tutorial.tutorialId!)),
      expect: () =>
          [ProjectSubmissionLoading(), TutorialFetching(), expectedState],
    );
  });
